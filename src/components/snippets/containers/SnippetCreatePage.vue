  ...
  export default {
    ...
    methods: {
      ...
      onSubmit() {
        if (this.isValid()) {
          const snippet = {
            title: this.newSnippetData.title.trim() || snippetData.DEFAULT_TITLE,
            url: this.newSnippetData.url.trim()
          };
      ...
      isValid() {
        ...
        // validate username
        params = {minLength: 3};
        const titleVal = validate(this.newSnippetData.title, params);
        if (!titleVal.valid) {
        ...
        const urlVal = validate(this.newSnippetData.url, params);
        if (!urlVal.valid) {
